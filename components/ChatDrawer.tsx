      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-4 bg-white border-t border-slate-100 flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 bg-slate-100 rounded-full border border-transparent focus:bg-white focus:border-emerald-500 focus:ring-0 outline-none text-sm transition-all"
            disabled={isTyping}
          />
          <button 
            type="submit"
            disabled={!input.trim() || isTyping}
            className="p-3 bg-emerald-600 text-white rounded-full hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-md shadow-emerald-200"
          >
            <Send size={18} />
          </button>
        </form>
